...
public class SkullBoss : MonoBehaviour, Enemy
{
    ...
    [Header("BGM設定")]
    [SerializeField] private AudioSource normalBGM; // 通常BGM
    [SerializeField] private AudioSource bossBGM; // ボスBGM
    ...
    private void StartBattle()
    {
        ...
        if (bossBGM != null)
        {
            Debug.Log("ボスBGMを再生します");
            // ボスBGMオブジェクトをアクティブにしてから再生
            bossBGM.gameObject.SetActive(true);
            bossBGM.Play();
        }
        else
        {
            Debug.LogWarning("ボスBGMが設定されていません");
        }
